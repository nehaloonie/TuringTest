...
public class TweetUploadService extends IntentService {
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        final long tweetID = intent.getLongExtra(EXTRA_TWEET_ID, 0L);
        uploadTweet(twitterSession, tweetText, imageUri, tweetID);
    }

    void uploadTweet(final TwitterSession session, final String text, final Uri imageUri, long tweetID) {
        if (imageUri != null) {
            uploadMedia(session, imageUri, new Callback<Media>() {
                @Override
                public void success(Result<Media> result) {
                    uploadTweetWithMedia(session, text, result.data.mediaIdString, 0);
                }
            ...
            uploadTweetWithMedia(session, text, null, tweetID);
        }
    }

    void uploadTweetWithMedia(TwitterSession session, String text, String mediaId, long tweetID) {
        final TwitterApiClient client = dependencyProvider.getTwitterApiClient(session);
        Log.d("Tweet ID : ", String.valueOf(tweetID));
        client.getStatusesService().update(text, tweetID, null, null, null, null, null, false, mediaId)
                .enqueue(
                        new Callback<Tweet>() {
                            @Override
