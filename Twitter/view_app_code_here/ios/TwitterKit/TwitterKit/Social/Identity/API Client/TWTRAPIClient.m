...
- (void)sendTweetWithText:(NSString *)tweetText completion:(TWTRSendTweetCompletion)completion
{
    ...
    NSMutableDictionary *parameters = [[self class] baseTweetQueryParametersByAppendingParameters:nil];
    parameters[@"status"] = tweetText;
    //parameters[@"in_reply_to_status_id"] = @"1012285832093425664";
...
- (void)sendTweetWithTextAndReplyID:(NSString *)tweetText replyId:(NSString *)replyid completion:(TWTRSendTweetCompletion)completion
{
    TWTRParameterAssertOrReturn(tweetText);
    TWTRParameterAssertOrReturn(completion);
...
{
    return @{@"include_cards": @"true", @"cards_platform": @"TwitterKit-13", @"auto_populate_reply_metadata":@"true"};
}
