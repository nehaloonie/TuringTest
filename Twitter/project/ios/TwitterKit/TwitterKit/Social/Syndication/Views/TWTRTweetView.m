...
@interface TWTRTweetView () <TWTRAttributedLabelDelegate, TWTRProfileHeaderViewDelegate, TWTRTweetMediaViewDelegate, UIGestureRecognizerDelegate, UITextViewDelegate, TWTRSubscriber , TWTRReplyButtonDelegate>
...
- (instancetype)initWithTweet:(TWTRTweet *)tweet style:(TWTRTweetViewStyle)style
{
    ...
    if (self) {
        ...
        // Reply Button
        _replyButton = [[TWTRReplyButton alloc] initWithReplyButtonSize:TWTRReplyButtonSizeRegular];
        _replyButton.hidden = (style == TWTRTweetViewStyleCompact);
        _replyButton.scribeViewName = TWTRScribeViewNameTweet;
        _replyButton.translatesAutoresizingMaskIntoConstraints = NO;
        _replyButton.delegate = self;
        [_actionContainer addSubview:_replyButton];
...
- (void)setShowActionButtons:(BOOL)showActionButtons
{
    ...
    self.replyButton.hidden = !showActionButtons;
...
- (void)configureWithTweet:(TWTRTweet *)tweet
{
    ...
    [self.replyButton configureWithTweet:tweet];
...
{
    NSDictionary *views = @{@"reply": self.replyButton, @"like": self.likeButton, @"share": self.shareButton};

    [TWTRViewUtil addVisualConstraints:@"H:[like(40)]-30-[reply(40)]-30-[share(40)]" options:NSLayoutFormatAlignAllTop | NSLayoutFormatAlignAllBottom views:views];
...
- (void)setPresenterViewController:(UIViewController *)presenterViewController
{
    ...
    self.replyButton.presenterViewController = self.presenterViewController;
...
- (void) replyTapped:(TWTRTweet *)tweet {
    [self.delegate tweetView:self didReplyTap:tweet];
}
@end
